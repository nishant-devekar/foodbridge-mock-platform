# FoodBridge mock platform — Version 6

**Opened 30 August 2026, from `v5` as its baseline.** Byte-for-byte the 30 August `v5` freeze
at the moment it was cut — 260 files, 9.5 MB, 12 module repos, 26 destinations — and unlike
`v5` it is *not* frozen. This is the working cut; edits land here.

Why `v5` and not `v4`: `v4` is a narrow, single-destination cut (Customer Management →
Stock Audit & Health, no sidebar). `v6` continues the whole platform, so it starts from the
last full-platform freeze — which is also the one carrying Delivery Management as a real
offline port rather than a crawl.

**Mobile is the target.** Work in this cut is done and reviewed at a phone viewport
(375×812) first; the desktop layout inherited from `v3` by way of `v5` still renders, but it
is not what this cut is being designed against.

`v5` is frozen and read-only. Nothing in `v5/` moves again; anything that would have changed
it changes here instead. The log below the line is `v5`'s, carried over so this cut knows
where its files came from — new entries for `v6` go above it.
...
# Inherited log — everything below happened in `v5`, before this cut opened
