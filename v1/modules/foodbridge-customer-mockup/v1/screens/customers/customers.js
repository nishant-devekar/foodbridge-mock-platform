/* ==========================================================================
   ...
   Provenance for every region:
     ...
     stock drawer              components/drawer/CustomerStockDrawer.jsx
...
(function () {
  ...
  function rowActionsHTML(cfg, c) {
    return `
      <div class="row-actions">
        ${cfg.showStock ? `<button class="icon-btn stock" data-act="stock" data-id="${c._id}" data-tip="Stock">${I.BarChart2}</button>` : ""}
  ...
  // CustomerStockDrawer + StockCountView + StockHistoryView.
  // The header carries the customer name as an inline pill; BOTH segment
  // buttons are windmill `<Button>` (primary green) — there is no muted state,
  // only the panel below changes.
  function openStockDrawer(c) {
    const history = SEED.stockCounts[c._id] || [];
    const initial = titleCase(nameOf(c)).charAt(0) || "C";
    const products = SEED.products || [];
    const counts = {};

    const drawer = openDrawer({
      large: true,
      title: `Customer Stock <span class="badge-name"><i>${esc(initial)}</i>${esc(titleCase(nameOf(c)))}</span>`,
      description: "Manage stock counts and history for this customer",
      headRow: true,
      body: `
        <div class="seg">
          <button data-view="count">${I.ClipboardList} Stock Count</button>
          <button data-view="history">${I.History} Stock History</button>
        </div>
        <div class="drawer-body" style="padding:0" data-panel></div>
        <div class="stock-foot" data-foot></div>`,
      footer: null,
    });

    const panel = $("[data-panel]", drawer);
    const foot = $("[data-foot]", drawer);
          ...
          <td>
            <div class="prod">
              ...
              <div>
                ...
                <p class="art">Art No: ${esc(p.artNo)}</p>
              </div>
            </div>
          </td>
          <td><span class="cat">${esc(p.category)}</span></td>
          <td class="r"><span class="sys">${p.systemStock}<span class="u">${esc(p.unit)}</span></span></td>
          <td class="r">
            <div class="stepper">
              <button type="button" data-step="-1">${I.Minus}</button>
              <input type="text" inputmode="numeric" placeholder="0" value="${v == null ? "" : v}">
              <button type="button" data-step="1">${I.FiPlus}</button>
            </div>
          </td>
          <td class="r"><span class="diff tdiff none">—</span></td>
        </tr>`;
        })
        .join("");

    // StockCountView renders a desktop TABLE (`hidden md:block`) and a mobile
    // CARD list (`md:hidden`) side by side in the DOM — not one reflowed
    // layout. Cards repeat the diff badge inline (top-right of the row) rather
      ...
      list
        ...
        <div class="pcard" data-p="${p.id}">
          <div class="pcard-top">
            ...
            <span class="diff cdiff none">—</span>
          </div>
          <div class="pcard-bottom">
            <span class="sys">System: ${p.systemStock} <span class="u">${esc(p.unit)}</span></span>
            <div class="stepper">
              <button type="button" data-step="-1">${I.Minus}</button>
              <input type="text" inputmode="numeric" placeholder="0" value="${v == null ? "" : v}">
              <button type="button" data-step="1">${I.FiPlus}</button>
    ...
    const diffText = (v, systemStock) => {
      if (v == null || v === "") return { cls: "none", html: "—" };
      const d = Number(v) - systemStock;
      if (d === 0) return { cls: "match", html: "Match" };
      return { cls: d > 0 ? "up" : "down", html: `${d > 0 ? "+" : ""}${d}` };
    };

    // Desktop table cell shows the unit alongside the delta; the mobile card
    // badge does not (ProductCard.jsx: `{diff===0?"Match":diff>0?`+${diff}`:diff}`,
    // no unit) — and desktop colours "over-counted" BLUE, not green (StockCountView.jsx
    // ProductRow: `diff>0 ? "text-blue-600" : "text-red-500"`; ProductCard uses the
    // pill equivalents bg-blue-50/text-blue-700 and bg-red-50/text-red-600).
    const paintDiff = (p) => {
      const v = counts[p.id];
      const { cls, html } = diffText(v, p.systemStock);
      panel.querySelectorAll(`[data-p="${p.id}"] .tdiff`).forEach((cell) => {
        cell.className = "diff tdiff " + cls;
        cell.innerHTML = cls === "up" || cls === "down"
          ? `${html}<span class="u" style="margin-left:4px;font-size:12px;opacity:.7">${esc(p.unit)}</span>`
          : html;
      });
      panel.querySelectorAll(`[data-p="${p.id}"] .cdiff`).forEach((cell) => {
        cell.className = "diff cdiff " + cls;
        cell.textContent = html;
      });
    };

    const paintCount = (filter) => {
      const q = (filter || "").trim().toLowerCase();
      const list = q
        ? products.filter(
            (p) =>
              p.name.toLowerCase().includes(q) || String(p.artNo).toLowerCase().includes(q),
          )
        : products;
      const entered = Object.values(counts).filter((v) => v !== "" && v != null).length;

      panel.innerHTML = `
        <div class="stock-body">
          <div class="stock-grid">
            <div>
              <label for="sc-date">Count Date &amp; Time</label>
              <input class="stock-field" id="sc-date" type="datetime-local" value="${new Date().toISOString().slice(0, 16)}">
            </div>
            <div>
              <label for="sc-notes">Notes <span class="opt">(optional)</span></label>
              <input class="stock-field" id="sc-notes" type="text" placeholder="e.g. Monthly audit — aisle 3 &amp; 4">
            </div>
          </div>
          ...
          <div class="stock-card">
            <div class="chead">
              <h2>Products</h2>
              <span class="count">${entered} / ${products.length} entered</span>
            </div>
            <div class="csearch">
              ${I.Search}
              <input type="text" id="sc-search" value="${esc(filter || "")}" placeholder="Search by product name or article number…">
            </div>
            ${
              list.length
                ? `<div class="stock-table-wrap" style="overflow-x:auto">
                     <table class="stock">
                       <thead><tr>
                         ...
                         <th class="r">System Stock</th><th class="r">Counted</th><th class="r">Difference</th>
                       </tr></thead>
                       <tbody>${rowsHTML(list)}</tbody>
                     </table>
                   </div>
                   <div class="stock-cards">${cardsHTML(list)}</div>`
                : `<div class="stock-empty">${I.Search}<p>No products found</p></div>`
            }
          </div>
        </div>`;

      foot.innerHTML = `
        <button class="btn-slate" data-cancel>Cancel</button>
        <button class="btn-save" data-save-count ${entered ? "" : "disabled"}>${I.Check} Save Stock Count</button>`;

      // Table and card are two independent renderings of the same `counts`
      // state (StockCountView mounts both, CSS toggles which is visible) — an
      // edit in either input keeps both, and both diff displays, in sync.
      list.forEach((p) => {
        const sync = (value) => {
          counts[p.id] = value === "" ? "" : String(Math.max(0, Number(value) || 0));
          panel.querySelectorAll(`[data-p="${p.id}"] input`).forEach((inp) => {
            if (inp.value !== counts[p.id]) inp.value = counts[p.id];
          });
          paintDiff(p);
          const entered2 = Object.values(counts).filter((v) => v !== "" && v != null).length;
          $(".chead .count", panel).textContent = `${entered2} / ${products.length} entered`;
          $("[data-save-count]", foot).disabled = entered2 === 0;
        };
        panel.querySelectorAll(`[data-p="${p.id}"] input`).forEach((inp) => {
          inp.oninput = () => sync(inp.value);
        });
        panel.querySelectorAll(`[data-p="${p.id}"] [data-step]`).forEach((b) => {
          b.onclick = () => sync(String(Math.max(0, Number(counts[p.id] || 0) + Number(b.dataset.step))));
        });
        if (counts[p.id] != null && counts[p.id] !== "") paintDiff(p);
      });

      const search = $("#sc-search", panel);
      search.oninput = debounce(() => {
        paintCount(search.value);
        const box = $("#sc-search", panel);
        box.focus();
        box.setSelectionRange(box.value.length, box.value.length);
      }, 200);

      $("[data-cancel]", foot).onclick = closeOverlays;
      $("[data-save-count]", foot).onclick = () => {
        const n = Object.values(counts).filter((v) => v !== "" && v != null).length;
        toast(`Stock count saved for ${n} product${n === 1 ? "" : "s"}.`);
        paintHistory();
        setActive("history");
      };
    };

    // StockHistoryView: a FROM/TO date range with an Apply button and a record
    // count, then one row per counting SESSION (date · products · comment) that
    // expands to the per-product Art No / Product / System / Counted breakdown.
    const paintHistory = () => {
      foot.innerHTML = "";
      const sessions = SEED.stockSessions?.[c._id] || [];
      ...
      panel.innerHTML = `
        <div class="stock-body">
          <div class="hist-bar">
            <div class="hf"><label>From</label><input class="stock-field" type="date" value="${from}"></div>
            <div class="hf"><label>To</label><input class="stock-field" type="date" value="${to}"></div>
            <button class="btn-apply" data-apply>Apply</button>
            <span class="hcount">${sessions.length} record${sessions.length === 1 ? "" : "s"}</span>
          </div>

          ${
            sessions.length
              ? `<div class="stock-card">
                   <table class="stock hist">
                     <thead><tr>
                       <th>Date</th><th class="c">Products</th><th>Comment</th><th></th>
                     </tr></thead>
                     <tbody>
                       ${sessions
                         .map(
                           (ses, i) => `
                         <tr class="srow" data-s="${i}">
                           <td>${esc(ses.at)}</td>
                           <td class="c">${ses.lines.length}</td>
                           <td class="cmt">${ses.comment ? esc(ses.comment) : '<span class="dash">—</span>'}</td>
                           <td class="r"><span class="chev">${I.ChevronDownLu}</span></td>
                         </tr>
                         <tr class="sdetail hidden" data-d="${i}">
                           <td colspan="4">
                             <div class="sdetail-box">
                               <table class="stock inner">
                                 <thead><tr>
                                   <th>Art No</th><th>Product</th><th class="r">System</th><th class="r">Counted</th>
                                 </tr></thead>
                                 <tbody>
                                   ${ses.lines
                                     .map(
                                       (l) =>
                                         `<tr><td class="art">${esc(l.artNo)}</td>
                                          <td class="pn">${esc(l.product)}</td>
                                          <td class="r">${l.system}</td>
                                          <td class="r strong">${l.counted}</td></tr>`,
                                     )
                                     .join("")}
                                 </tbody>
                               </table>
              ...
              : `<div class="stock-card"><div class="stock-empty">${I.History}
                   <p>No stock count records found</p>
                   <span style="font-size:12px">Try adjusting the date range</span>
                 </div></div>`
          }
        </div>`;

      panel.querySelectorAll(".srow").forEach((tr) => {
        tr.onclick = () => {
          const d = $(`[data-d="${tr.dataset.s}"]`, panel);
          const open = d.classList.toggle("hidden");
          $(".chev", tr).innerHTML = open ? I.ChevronDownLu : I.ChevronUp;
        };
      });
      const apply = $("[data-apply]", panel);
      if (apply) apply.onclick = () => toast("Date range applied.", "info");
    };

    const setActive = (view) => {
      if (view === "count") paintCount("");
      else paintHistory();
    };

    drawer.querySelectorAll("[data-view]").forEach((b) => (b.onclick = () => setActive(b.dataset.view)));
    setActive("count");
  }
