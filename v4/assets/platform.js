...
(function () {
  ...
  function renderMobileHeader(config) {
    ...
    return (
      ...
      // No burger: there is no drawer for it to open in this cut. The brand
      // takes the space it had, which is where a one-screen app's title belongs
      // anyway.
      '<a href="#/" class="fb-mhead-brand fb-mhead-brand-lead" title="' + name + '">' +
  ...
  function renderShell(config) {
    return (
      ...
      // NO SIDEBAR IN THIS CUT.
      // The full platform renders two here — a desktop `aside` and a mobile
      // drawer with its backdrop — both filled by renderSidebarContent. This
      // version has exactly one destination, so a sidebar would be a list of
      // one item and a drawer that opens onto the screen you are already on.
      // Both are gone, along with the burger in the mobile header and the
      // burger mask over the module's own, since neither has anything left to
      // toggle. renderSidebarContent and the toggle wiring stay where they are:
      // nothing calls them now, and deleting them would make this file diverge
      // from the platform's for no gain when the two are next compared.

      '<div class="flex flex-col flex-1 w-full min-w-0">' +
      '<div class="fb-viewport" data-viewport>' +
      // The burger mask is still here, and still masks. The module draws its
      // OWN hamburger in its header and it is as dead as it ever was — it
      // toggles the module's sidebar, which clipLeft has removed. What changed
      // is that the mask no longer toggles anything either, so it is inert
      // rather than a control: `data-sidebar-toggle` is gone, the cover is not.
      '<button type="button" class="fb-burger-mask" data-burger-mask hidden aria-hidden="true" tabindex="-1"></button>' +
